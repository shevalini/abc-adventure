...
export default function AlphabetCard({ letter, letterProgress }) {
  ...
  return (
    <button
      className="letter-card bg-white rounded-2xl p-4 flex flex-col items-center gap-1.5 shadow-sm
                 border-2 focus-visible:outline-2 focus-visible:outline-offset-2 relative overflow-hidden"
      style={{
    ...
    >
      {/* Completion badge */}
      {completed && (
        <span className="absolute top-2 right-2 animate-bounce-in" aria-hidden="true">
          <CheckCircle2
            className="w-5 h-5"
